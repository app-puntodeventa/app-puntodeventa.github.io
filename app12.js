...
`;
  ...
  venta.items.forEach(item => {
    const linea = `${item.texto}`.padEnd(30, '.');
...
document.getElementById("btnDescargarQR").onclick = () => {
  ...
  link.download = `comprobante-${ultimaVenta.id || Date.now()}.png`;
  link.click();
};
...
function renderVenta(v) {
  ...
  div.innerHTML = `
    ...
    <div class="flex gap-2 mt-3">
      ...
      <button class="bg-purple-500 hover:bg-purple-600 text-white px-3 py-1 rounded text-sm transition btnCompartir">📤 Compartir</button>
