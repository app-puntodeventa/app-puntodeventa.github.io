// ======================================
// 🔐 SISTEMA DE LICENCIA (OCULTO)
// ======================================

const MASTER_LICENSE_KEY = "POS2024TITANIUM888"; // Clave maestra oculta
const DEMO_LIMIT = 3; // Máximo de ventas en versión demo
...
function checkLicenseStatus() {
  const licenseData = localStorage.getItem("pos_license");
  if (licenseData === "active") return true;
  return false;
}
...
function checkDemoLimit() {
  ...
  const ventasCount = getVentasCount();
  return ventasCount < DEMO_LIMIT;
}
...
// ======================================
// 🧪 PARSER INTELIGENTE ULTRA AVANZADO - MODO BETA
// ======================================

function parserBetaAvanzado(texto) {
  if (!texto.trim()) return null;

  let t = texto.toLowerCase().trim();

  // Patrones ultra inteligentes
  const patrones = [
    // Patrón 1: "40 pesos de salchicha" o "40 pesos por salchicha"
    /^(\d+(?:\.\d+)?)\s*(?:pesos?|mxn|pesos?\s+por|por|a)\s+(.+?)$/i,

    // Patrón 2: "3 hojas por 1 peso" - detecta cantidad de producto por precio total
    /^(\d+(?:\.\d+)?)\s+(kg|g|l|litro|litros|pieza|pz|piezas|metro|m|docena|doc|pack|caja|bolsa|lata|bolsas|cajas|latas|kilo|kilos|hojas?)\s+por\s+(\d+(?:\.\d+)?)\s*(?:pesos?|mxn)?$/i,

    // Patrón 3: "3 peluches por 35 pesos" - cantidad de items por precio total
    /^(\d+(?:\.\d+)?)\s+(.+?)\s+por\s+(\d+(?:\.\d+)?)\s*(?:pesos?|mxn)?$/i,

    // Patrón 4: Cantidad unidad producto a precio (cantidad por unidad)
    /^(\d+(?:\.\d+)?)\s*(kg|g|l|litro|litros|pieza|pz|piezas|metro|m|docena|doc|pack|caja|bolsa|lata|bolsas|cajas|latas|kilo|kilos)?\s+(?:de\s+)?(.+?)\s+a\s+(\d+(?:\.\d+)?)$/i,

    // Patrón 5: "producto 25" - producto y precio por unidad (cantidad implícita = 1)
    /^(.+?)\s+(\d+(?:\.\d+)?)$/i
  ];
  ...
  for (let i = 0; i < patrones.length; i++) {
    ...
    // Patrón 1: "40 pesos de salchicha"
    if (i === 0 && match.length === 3) {
      return {
        cantidad: 1,
        unidad: "pieza",
        producto: match[2].trim(),
        precio: parseFloat(match[1]),
        precioTotal: parseFloat(match[1]),
        esPrecioTotal: true,
        esValido: true
      };
    }

    // Patrón 2: "3 hojas por 1 peso" 
    if (i === 1 && match.length === 4) {
      const cantidad = parseFloat(match[1]);
      const unidad = match[2].toLowerCase();
      const precioTotal = parseFloat(match[3]);
      const precioPorUnidad = (precioTotal / cantidad).toFixed(2);

      return {
        cantidad: 1,
        unidad: "pieza",
        producto: `${cantidad} ${unidad}`,
        precio: parseFloat(precioPorUnidad),
    ...
    // Patrón 3: "3 peluches por 35 pesos"
    ...
    // Patrón 4: "3kg huevo a 20"
    if (i === 3 && match.length === 5) {
      const cantidad = parseFloat(match[1]);
    ...
    // Patrón 5: "producto 25"
    if (i === 4 && match.length === 3) {
      return {
        cantidad: 1,
        unidad: "pieza",
        producto: match[1].trim(),
        precio: parseFloat(match[2]),
        precioTotal: parseFloat(match[2]),
        esPrecioTotal: false,
        esValido: true
      };
    }
...
if (inputBeta) {
  inputBeta.addEventListener("input", () => {
    const resultado = parserBetaAvanzado(inputBeta.value);
    ...
    const { cantidad, unidad, producto, precio, precioTotal } = resultado;
    ...
    previewBeta.innerHTML = `
      <div class="space-y-1">
        ...
        <div class="flex justify-between text-sm">
          <span class="text-gray-600">Precio c/u:</span>
          <span class="font-bold">$${precio.toFixed(2)}</span>
        </div>
        <div class="flex justify-between border-t pt-1">
          <span class="text-gray-600 font-bold">Total:</span>
          <span class="font-bold text-green-600 text-lg">$${precioTotal.toFixed(2)}</span>
        </div>
      </div>
    `;
  });
...
if (btnAgregarBeta) {
  btnAgregarBeta.onclick = () => {
    ...
    const { producto, precio, precioTotal } = resultado;

    const productoObj = actualizarProductoInventario(producto, precio, "pieza", 0);

    let gananciaEstimada = 0;
    if (productoObj.costo && productoObj.costo > 0) {
      gananciaEstimada = precioTotal - (productoObj.costo);
    }
    ...
    navigator.vibrate?.(30);
    console.log(`✅ Producto agregado: ${producto} @ $${precioTotal}`);
  };
}
...
if (btnFinalizar) {
  btnFinalizar.onclick = () => {
    ...
    // Verificar límite de demo después de cada venta
    if (!checkDemoLimit()) {
      setTimeout(() => {
        document.getElementById("licenseModal").classList.add("active");
      }, 500);
...
const btnValidarLicencia = document.getElementById("btnValidarLicencia");
const licenseInput = document.getElementById("licenseInput");

if (btnValidarLicencia) {
  btnValidarLicencia.onclick = () => {
    const code = licenseInput.value.trim();

    if (!code) {
      alert("❌ Ingresa un código de licencia");
      return;
    }

    if (validateLicense(code)) {
      activateLicense();
      alert("✅ ¡Licencia activada! Acceso ilimitado");
    } else {
