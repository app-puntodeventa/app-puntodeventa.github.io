...
// ======================================
// 🧾 COMPROBANTE - SIN QR AUTOMÁTICO
// ======================================
// NOTA: Si quieres agregar un generador QR en el futuro, 
// descomenta la sección de QR más abajo y usa una librería 
// alternativa como: https://cdn.jsdelivr.net/npm/qrcode@1.5.3/build/qrcode.min.js
...
function mostrarComprobanteQR(venta) {
  ...
  // Mostrar texto del comprobante en formato bonito
  comprobanteEl.textContent = comprobanteTexto;
  comprobanteEl.classList.remove("qr-mode");
  comprobanteEl.style.display = "block";
  
  // Ocultar contenedor de QR
  qrContainer.classList.add("hidden");

  document.getElementById("modalComprobante").showModal();
  
  console.log("✅ Comprobante listo para fotografiar");
}

// ======================================
// CÓDIGO COMENTADO PARA AGREGAR QR EN FUTURO
// ======================================
/*
// Si en el futuro quieres usar QR, descomentar esto:
// 1. Agregar en index15.html en <head>:
//    <script src="https://cdn.jsdelivr.net/npm/qrcode@1.5.3/build/qrcode.min.js"></script>
//
// 2. Descomentar la función abajo:

function mostrarComprobanteQRConQR(venta) {
  const comprobanteTexto = generarTextoComprobante(venta);
  const comprobanteEl = document.getElementById("comprobanteTexto");
  const qrContainer = document.getElementById("qrCode");
  ...
  // Hacer clickeable el comprobante para generar QR
...
document.getElementById("btnDescargarQR").onclick = () => {
  const comprobanteEl = document.getElementById("comprobanteTexto");
  if (!comprobanteEl || !comprobanteEl.textContent) {
    alert("⚠️ No hay comprobante para descargar.");
    return;
  }
  ...
  try {
    ...
    const archivo = new Blob([comprobanteEl.textContent], { type: 'text/plain' });
    elemento.href = URL.createObjectURL(archivo);
    elemento.download = `comprobante-${ultimaVenta?.usuario || "venta"}-${Date.now()}.txt`;
    elemento.click();
  } catch (error) {
...
console.log("✅ POS Pro v15 - Sistema iniciado. Comprobante listo para fotografiar");
