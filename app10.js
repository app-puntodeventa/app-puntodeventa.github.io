...
document.getElementById("btnInventario").onclick = () => {
  alert("📦 Inventario\n\nProductos totales: " + inventarioLocal.length + "\n\n(Función disponible en próxima versión)");
};
...
document.getElementById("btnReporte").onclick = () => {
  ...
  if (ventas.length === 0) {
    alert("Sin ventas para generar reporte");
    return;
  }
  ...
  doc.text(`Fecha: ${new Date().toLocaleString()}`, 10, y + 5);
  doc.text(`Total de ventas: ${ventas.length}`, 10, y + 10);
  ...
  ventas.forEach((v, idx) => {
    ...
    doc.text(`Venta #${idx + 1}`, 10, y);
    doc.setFont(undefined, "normal");
    doc.text(v.fecha, 10, y + 5);
    doc.text(`Total: $${v.total.toFixed(2)}`, 10, y + 10);
  ...
  doc.setFont(undefined, "bold");
  doc.setFontSize(14);
  doc.text(`TOTAL GENERAL: $${total.toFixed(2)}`, 105, y, { align: "center" });
