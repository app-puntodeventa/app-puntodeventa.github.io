...
if (searchProducto) {
  searchProducto.addEventListener("input", () => {
    ...
    opciones.forEach((opt, idx) => {
      ...
      opt.style.display = texto.includes(query) ? "block" : "none";
    });

    if (query) {
      const match = Array.from(opciones).find(opt => 
        opt.textContent.toLowerCase().includes(query)
      );
      if (match) selectProducto.value = match.value;
    }
  });
}
...
function parseIA(texto) {
  ...
  // Eliminador de palabras comunes
  t = t.replace(/\b(de|por|a|el|la|los|las|un|una|unos|unas|precio|costo|total|sale|vale)\b/gi, " ");
  t = t.replace(/\s+/g, " ").trim();

  // Patrones inteligentes (orden de especificidad)
  const patrones = [
    // "40 pesos salchicha" o "40 pesos de salchicha"
    {
      regex: /^(\d+(?:\.\d+)?)\s*(?:pesos|mxn|mex|$)?(?:\s+(.+))?$/i,
      parse: (m) => ({
        cantidad: 1,
        producto: m[2] ? m[2].trim() : "producto",
    ...
    // "3 hojas por 1 peso" - cantidad unidad por precio total
    {
      regex: /^(\d+(?:\.\d+)?)\s*(kg|g|l|litro|litros|pieza|pz|piezas|metro|m|docena|doc|pack|caja|bolsa|lata|hojas?|unidad|unidades|ración|raciones|tazas?|cucharas?|gramos?|kilos?)\s+(.+?)\s+por\s+(\d+(?:\.\d+)?)\s*(?:pesos?|mxn)?$/i,
      parse: (m) => ({
        cantidad: 1,
        producto: `${m[1]} ${m[2]} ${m[3]}`.trim(),
        precioTotal: parseFloat(m[4]),
        precioPorUnidad: parseFloat(m[4]),
        esTotal: true
      })
    },

    // "3 peluches por 35 pesos" - cantidad producto por precio total
    {
      regex: /^(\d+(?:\.\d+)?)\s+(.+?)\s+por\s+(\d+(?:\.\d+)?)\s*(?:pesos?|mxn)?$/i,
      parse: (m) => ({
        cantidad: 1,
        producto: `${m[1]} ${m[2]}`.trim(),
        precioTotal: parseFloat(m[3]),
        precioPorUnidad: parseFloat(m[3]),
        esTotal: true
      })
    },

    // "3kg huevo a 20" - cantidad unidad producto a precio unitario
    {
      regex: /^(\d+(?:\.\d+)?)\s*(kg|g|l|litro|litros|pieza|pz|piezas|metro|m|docena|doc|pack|caja|bolsa|lata|hojas?|unidad|unidades|ración|raciones|tazas?|cucharas?|gramos?|kilos?)\s+(.+?)\s+a\s+(\d+(?:\.\d+)?)$/i,
      parse: (m) => ({
        cantidad: parseFloat(m[1]),
        producto: m[3].trim(),
        precioPorUnidad: parseFloat(m[4]),
        precioTotal: parseFloat(m[1]) * parseFloat(m[4]),
        unidad: m[2].toLowerCase(),
        esTotal: false
      })
    },

    // "2 pan a 5" - cantidad producto a precio unitario
    {
      regex: /^(\d+(?:\.\d+)?)\s+(.+?)\s+a\s+(\d+(?:\.\d+)?)$/i,
      parse: (m) => ({
        cantidad: parseFloat(m[1]),
        producto: m[2].trim(),
        precioPorUnidad: parseFloat(m[3]),
        precioTotal: parseFloat(m[1]) * parseFloat(m[3]),
        esTotal: false
      })
    },

    // "producto 25" - producto a precio
    {
      regex: /^(.+?)\s+(\d+(?:\.\d+)?)$/i,
      parse: (m) => ({
        cantidad: 1,
        producto: m[1].trim(),
        precioPorUnidad: parseFloat(m[2]),
        precioTotal: parseFloat(m[2]),
        esTotal: false
      })
    }
  ];
  ...
  for (let patron of patrones) {
    ...
    if (match) {
      ...
      if (resultado.producto && resultado.precioTotal > 0) {
        return { ...resultado, esValido: true };
      }
    }
  }
    ...
    previewBeta.innerHTML = `
      <div class="space-y-1">
        <div class="flex justify-between text-sm">
          ...
          <span class="font-bold">${resultado.producto.toUpperCase()}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-gray-600">💰 P/U:</span>
          <span class="font-bold">$${resultado.precioPorUnidad.toFixed(2)}</span>
        </div>
        <div class="flex justify-between border-t pt-1 font-bold">
          <span>Total:</span>
          <span class="text-green-600 text-lg">$${resultado.precioTotal.toFixed(2)}</span>
        </div>
      </div>
    `;
  });
...
if (btnAgregarBeta) {
  btnAgregarBeta.onclick = () => {
    ...
    const { producto, precioPorUnidad, precioTotal } = resultado;
    const productoObj = actualizarProductoInventario(producto, precioPorUnidad, "pieza", 0);

    let ganancia = 0;
    if (productoObj.costo && productoObj.costo > 0) {
      ganancia = precioTotal - (productoObj.costo);
    }
...
console.log("✅ POS Pro v1.0 - Sistema iniciado");
